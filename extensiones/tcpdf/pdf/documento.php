	...
	public function traerImpresionFactura()
	{
		...
		switch ($this->documento) {
			...
			case "Cotizacion_Afecta":
				...
				// Verificar si la región es un ID o un nombre
				if (is_numeric($receptor["region"])) {
					$region = ControladorRegiones::ctrMostrarRegiones("id", $receptor["region"]);
					$regionNombre = $region ? $region["nombre"] : "Desconocido"; // Manejo de errores si no se encuentra
				} else {
					$regionNombre = $receptor["region"];
				}

				// Verificar si la comuna es un ID o un nombre
				if (is_numeric($receptor["comuna"])) {
					$comuna = ControladorRegiones::ctrMostrarComunas("id", $receptor["comuna"]);
					$comunaNombre = $comuna ? $comuna["nombre"] : "Desconocido"; // Manejo de errores si no se encuentra
				} else {
					$comunaNombre = $receptor["comuna"];
				}
		...
		$pdf->SetFont('helvetica', '', 8);
		$pdf->Cell(120, 7, $regionNombre, "L", 0, 'L');
		$pdf->SetFont('helveticaB', '', 8);
		$pdf->Cell(20, 7, 'Comuna:', 1, 0, 'L');
		$pdf->SetFont('helvetica', '', 8);
		$pdf->Cell(0, 7, $comunaNombre, 'R', 1, 'L');
